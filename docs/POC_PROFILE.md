...
| Vessel naming | Carousel 10, R8 — **decided** (SCP-0002) | Vessel names the assembled Consumer Runtime and its outward product boundary; Carousel remains the only deduction engine. CLI, MCP, service, and library entry points are Vessel adapters, while component APIs remain explicitly component surfaces. Repository, module, and binary renaming is planned separately | — |
...
| Voyage result and incremental reuse | SCP-0004 core and publication-time membership — **decided**; remaining details under discussion; not implemented | This POC returns the Root outcome and trace only. It does not yet publish a Fully Touchdown Cable, provenance sidecar, dependency fingerprints, or `reusedFrom` records, and it never skips Host evaluation as a reuse optimization | [VESSEL_PLAN.md](VESSEL_PLAN.md) |
